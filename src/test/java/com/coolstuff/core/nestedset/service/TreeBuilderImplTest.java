...
class TreeBuilderImplTest {
    ...
    @Test
    void buildTree() throws InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        nodeList.add(root);
        nodeList.add(electronic);
        nodeList.add(computer);
        nodeList.add(mobile);

        NodeComponentFactory factory = mock(NodeComponentFactory.class);

        when(factory.createCompositeNodeComponent()).thenReturn(mock(NodeComponent.class));
        when(factory.createLeafNodeComponent()).thenReturn(mock(NodeComponent.class));

        TreeBuilderImpl treeBuilder = new TreeBuilderImpl(factory);

        var tree = treeBuilder.buildTree(nodeList);
        Assertions.assertEquals( "Root",tree.getName());
        Assertions.assertEquals(1, tree.getChildren().size());
        Assertions.assertEquals( "Electronic", tree.getChildren().stream().findFirst().get().getName());
    }
